...
public class CostComputationModule {
    private double COST_INFEASIBLE;
    private boolean DEBUG;
    private int MAX_WT;
    private int MAX_DT;
    
    public CostComputationModule(double _cost_infeasible, boolean _debug, int _max_wt, int _max_dt)
    {
        COST_INFEASIBLE = _cost_infeasible;
        DEBUG = _debug;
        MAX_WT = _max_wt;
        MAX_DT = _max_dt;
    }

    public double[][] getCostMatrix(int[][] rb, int[] vehicles, Integer[][] context_map, Communicator communicator, Tools tools, ConcurrentHashMap<Integer, Integer> lut, ConcurrentHashMap<Integer, Integer> luv,
                                        ConcurrentHashMap<Key<Integer,Integer>,int[]> cache_w, ConcurrentHashMap<Key<Integer,Integer>,int[]> cache_b, PathComputationModule pcm) throws ClientException {
        ...
        for (int i = 0; i < num_customers; i++) {
          ...
          for (int j = 0; j < num_vehicles; j++) {
            ...
            if (Arrays.stream(context_map[i]).anyMatch(x -> x == vehicle_id) ) {
              weight_matrix[i][j]=getInsertionCost(rb[i], vehicle_id, communicator, tools, lut, luv, cache_w, cache_b, pcm);
            } else {
    ...
    protected double getInsertionCost(final int[] r, final int sid, Communicator communicator, Tools tools, ConcurrentHashMap<Integer, Integer> lut, ConcurrentHashMap<Integer, Integer> luv,
                                        ConcurrentHashMap<Key<Integer,Integer>,int[]> cache_w, ConcurrentHashMap<Key<Integer,Integer>,int[]> cache_b, PathComputationModule pcm) throws ClientException {
        ...
        try{
          ...
          final int capacity = communicator.queryServerCapacity(sid);
          if (DEBUG) {
          ...
          final int now = communicator.retrieveClock();
          ...
          int[] brem = communicator.queryServerScheduleRemaining(sid, now);
          /*
          if (DEBUG) {
          ...
          */
          ...
          final int[] wact = communicator.queryServerRouteActive(sid);
          /*
          if (DEBUG) {
          ...
          */
          ...
          int[] direct_route = tools.computeRoute(ro, rd, re);
          ...
          for (int i = 0; i < imax; i++) {
            ...
            for (int j = i; j < jmax; j++) {
              ...
              ok = (communicator.queryServerCapacityViolations(sid, rq, tbeg, tend)[0] == 0);
              if (DEBUG) {
              ...
              */
              ...
              int[] wnew = null;
              {
                ...
                int[] _leg = pcm.getPath(wbeg[1], bnew[1], wbeg[0], tools);
                int _n = _leg.length;
                int _t = _leg[(_n - 2)];
                ...
                for (int _i = 1; _i < _p; _i++) {
                  ...
                  _leg = pcm.getPath(_u, _v, _t, tools);
                  _legs[_i] = _leg;
                  // Update _n and _t
                  _n += (_leg.length - 2);
                ...
                */
                ...
                for (int _i = 0; _i < (bnew.length - 3); _i += 4) {
                  ...
                  if (_rid != 0) {//rid is only 0 at the end point of the route
                    int[] _u = communicator.queryUser(_rid);
                    int _ue = _u[2];
                    int _ul = _u[3];
          ...
          if (feasible && cmin>=0) {
            ...
            cache_w.put(k_temp,wmin);
            cache_b.put(k_temp,bmin);
